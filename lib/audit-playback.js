...
function getPlaybackByBatch(batchId, viewer) {
  const data = store.read();
  const batchLogs = data.revisionLogs.filter(l => l._importBatchId === batchId);
  if (batchLogs.length === 0) return [];

  const batchLogIds = new Set(batchLogs.map(l => l.id));
  let records = (data.playbackRecords || []).filter(r =>
    r.logIds && r.logIds.some(id => batchLogIds.has(id))
  );

  if (viewer) {
    records = records.map(r => {
      if (r.playbackBy === viewer || authSvc.canApproveAndPublish(viewer)) {
        return r;
      }
      return {
        id: r.id,
        playbackAt: r.playbackAt,
        playbackBy: r.playbackBy,
        logCount: r.logCount,
        summary: r.summary,
        _redacted: true
      };
