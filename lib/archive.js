...
function exportRevisionLog(docId, filters = {}) {
  ...
  if (filters.status) {
    const statusActionMap = {
      ...
      published: ['publish'],
      withdrawn: ['withdraw'],
      draft: ['draft_save', 'draft_delete', 'draft_snapshot_restore', 'draft_snapshot_restore_conflict', 'draft_snapshot_delete']
    };
...
function exportRevisionLogCSV(docId, filters = {}) {
  ...
  const actionLabels = {
    ...
    draft_save: '保存草稿',
    draft_delete: '删除草稿',
    draft_snapshot_restore: '恢复草稿快照',
    draft_snapshot_restore_conflict: '草稿快照恢复冲突拦截',
    draft_snapshot_delete: '删除草稿快照'
  };
