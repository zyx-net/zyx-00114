...
function importVaultAuditPackage(packageData, importer, options = {}) {
  ...
  return store.update(data => {
    ...
    const existingPackage = (data.vaultImportPackages || []).find(
      p => p.fingerprint === packageFingerprint
    );
    ...
    if (existingVaultBatch) {
      conflicts.push({
        type: 'VAULT_BATCH_DUPLICATE',
        message: `保险箱批次 ${sourceVaultBatchId.slice(0, 8)}... 已存在，所有者: ${existingVaultBatch.ownerId}`,
        existingOwner: existingVaultBatch.ownerId,
        importOwner: packageData.vaultBatch?.ownerId
      });
    }

    const newVaultBatchId = existingVaultBatch ? uuidv4() : sourceVaultBatchId;

    if (!existingVaultBatch) {
      const newVaultBatch = {
        ...packageData.vaultBatch,
        vaultBatchId: newVaultBatchId,
      ...
      imported.push({ type: 'vaultBatch', id: newVaultBatchId });
    } else {
      conflicts.push({
        type: 'VAULT_BATCH_EXISTS',
        message: `保险箱批次已存在，使用新ID ${newVaultBatchId.slice(0, 8)}... 导入`,
        newId: newVaultBatchId,
        existingId: sourceVaultBatchId
      });
    }
    ...
    if (packageData.sourceBatch) {
      ...
      if (!existingSourceBatch || existingSourceBatch._redacted) {
        ...
        conflicts.push({
          type: 'SOURCE_BATCH_EXISTS',
          message: `源批次 ${packageData.sourceBatch.batchId.slice(0, 8)}... 已存在，跳过导入`,
          existingBatchId: packageData.sourceBatch.batchId
        });
      }
    }
