...
function canCreateVaultBatch(operator) {
  if (!operator) return false;
  return canApproveAndPublish(operator);
}

function canViewVaultDetail(vaultOwnerId, viewer) {
  if (!viewer) return false;
  if (vaultOwnerId === viewer) return true;
  return canApproveAndPublish(viewer);
}

function canExportVaultAudit(vaultOwnerId, viewer) {
  if (!viewer) return false;
  if (vaultOwnerId === viewer) return true;
  return canApproveAndPublish(viewer);
}

function canImportVaultPackage(operator) {
  if (!operator) return false;
  return canApproveAndPublish(operator);
}

function canViewVaultAccessTrail(vaultOwnerId, viewer) {
  if (!viewer) return false;
  if (vaultOwnerId === viewer) return true;
  return canApproveAndPublish(viewer);
}

function canUpdateVaultNotes(vaultOwnerId, viewer) {
  if (!viewer) return false;
  return vaultOwnerId === viewer;
}

function canPlaybackVaultBatch(vaultOwnerId, viewer) {
  if (!viewer) return false;
  if (vaultOwnerId === viewer) return true;
  return canApproveAndPublish(viewer);
}

function canApplyForSensitiveGrant(viewer) {
  if (!viewer) return false;
  return true;
}

function canApproveSensitiveGrant(vaultOwnerId, approver) {
  if (!approver) return false;
  if (vaultOwnerId === approver) return true;
  return canApproveAndPublish(approver);
}

function canRevokeSensitiveGrant(vaultOwnerId, operator) {
  if (!operator) return false;
  if (vaultOwnerId === operator) return true;
  return canApproveAndPublish(operator);
}

function canViewSensitiveGrantDetail(applicant, viewer) {
  if (!viewer) return false;
  if (applicant === viewer) return true;
  return canApproveAndPublish(viewer);
}

function canImportSensitivePackage(operator) {
  if (!operator) return false;
  return canApproveAndPublish(operator);
}
...
module.exports = {
  ...
  canViewVaultDetail,
  canExportVaultAudit,
  canImportVaultPackage,
  canViewVaultAccessTrail,
  canUpdateVaultNotes,
  canPlaybackVaultBatch,
  canApplyForSensitiveGrant,
  canApproveSensitiveGrant,
  canRevokeSensitiveGrant,
  canViewSensitiveGrantDetail,
  canImportSensitivePackage
};
