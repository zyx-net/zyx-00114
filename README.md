...
新增「**敏感审计借阅台**」模块，把脱敏摘要浏览、临时授权申请、完整明细解锁、导出留痕串成一条能独立验收的链路。

**核心设计原则**：
- **默认脱敏**：普通角色（非 owner 非审批员）只能看到摘要、状态和最小元数据；批次号、日志明细、备注、关联操作等敏感信息全部脱敏
- **一次性授权单**：只有 owner 或拿到一次性授权单的人，才能在有效期内查看完整明细；授权单到期或撤销后，已打开的会话也立刻失效
- **授权单全生命周期**：支持创建、审批、撤销、到期回收、原因记录和时长上限配置
- **重启一致性**：授权单、会话、权限结果全部持久化，服务重启后权限不漂移
- **导入冲突检测**：审计包导出再导入，重复导入同一批内容时明确提示冲突并保留处理日志
- **权限绕过防护**：旧缓存、筛选参数和组合查询都不能绕过权限

**6.10.1 授权单实体**

每次申请创建一条授权单，落库以下信息：
- `grantId`：授权单 UUID
- `targetVaultBatchId`：目标保险箱批次 ID
- `applicant`：申请人
- `approver`：审批人（审批后填充）
- `reason`：申请原因
- `status`：状态（pending/approved/revoked/expired）
- `createdAt` / `approvedAt` / `revokedAt`：时间戳
- `expiresAt`：过期时间（审批时 = approvedAt + durationMinutes）
- `durationMinutes`：授权时长（分钟），受 maxDurationMinutes 上限约束
- `revokeReason`：撤销原因
- `requestedDurationMinutes`：申请时长（可能被截断）
- `maxDurationMinutes`：时长上限

**6.10.2 权限控制矩阵**

| 操作 | owner/审批员 | 持有授权单的编辑员 | 普通编辑员 | 匿名 |
|---|---|---|---|---|
| 申请授权 | ❌（已授权无需申请） | - | ✅ | ❌ |
| 审批授权 | ✅ | ❌ | ❌ | ❌ |
| 撤销授权 | ✅ | ❌ | ❌ | ❌ |
...
| 查看批次详情（脱敏） | - | - | ⚠ | ⚠ |
| 查看日志明细 | ✅ | ✅（授权有效期内） | ⚠（脱敏） | ⚠（脱敏） |
| 查看备注 | ✅ | ✅（授权有效期内） | ⚠（null） | ⚠（null） |
| 导出审计包 | ✅ | ✅（授权有效期内） | ❌ | ❌ |
| 导入审计包 | ✅ | ❌ | ❌ | ❌ |

**脱敏规则**：与保险箱模块一致，使用 `applyDeskRedaction(obj, level)` 函数，summary 级别保留元数据、移除敏感字段；full 级别仅保留 ID 和状态。

**6.10.3 授权单生命周期**

1. **申请**：编辑员通过 `POST /api/sensitive-desk/apply` 提交申请，状态变为 `pending`
2. **审批**：owner 或审批员通过 `POST /api/sensitive-desk/:grantId/approve` 审批，状态变为 `approved`，计算 `expiresAt`
3. **使用**：审批通过后，申请人通过 `POST /api/sensitive-desk/:grantId/open-session` 开启会话，用 `grantId` 访问完整明细
4. **撤销**：owner 或审批员通过 `POST /api/sensitive-desk/:grantId/revoke` 撤销，相关会话立刻失效
5. **过期**：系统自动检测过期授权，会话标记为 `invalid`；也可手动调用 `POST /api/sensitive-desk/expire-grants` 触发

**6.10.4 会话管理**

- 每个授权单可开启多个会话，会话记录 `sessionId`、`grantId`、`userId`、`createdAt`、`lastAccessedAt`
- 每次数据访问都通过 `validateSession` 重新校验授权单有效性
- 授权单撤销或过期后，关联的所有会话立刻标记为 `invalid`
- 撤销后已打开会话也立刻失效：后续任何数据访问请求都会被拒绝

**6.10.5 审计包导出与导入**

**导出**（`GET /api/sensitive-desk/vault-batches/:vaultBatchId/export`）：
- 与保险箱导出类似，额外包含授权单信息和导出方式（grant/owner）
- 指纹验证、篡改检测与保险箱一致

**导入**（`POST /api/sensitive-desk/import`）：
- 指纹校验、重复检测、冲突策略与保险箱一致
- 额外处理授权单的导入：已有授权单按 ID 去重，冲突类型为 `GRANT_DUPLICATE`
- 冲突策略：`reject`（默认返回 409）、`skip`（跳过）、`force`（强制导入）

**6.10.6 防止权限绕过的措施**

1. **每次请求实时校验**：不依赖缓存，每次数据访问都重新检查授权单状态
2. **会话绑定授权单**：会话的有效性完全取决于关联授权单的状态
3. **筛选参数不影响权限**：无论查询参数如何组合，非授权用户始终只拿到脱敏结果
4. **grantId 校验严格**：授权单的申请人必须与当前 viewer 一致，且目标批次必须匹配
5. **时长上限**：申请时长超过 `maxDurationMinutes` 时自动截断

**6.10.7 配置管理**

- `GET /api/sensitive-desk/config`：查看当前配置
- `PUT /api/sensitive-desk/config`：更新配置（仅审批员）
- `maxDurationMinutes`：授权时长上限，默认 120 分钟
- `defaultDurationMinutes`：默认授权时长，默认 30 分钟

**6.10.8 访问日志**

所有借阅台操作都记录到 `sensitiveDeskAccessLogs[]`：
- `action`：apply / approve / revoke / open_session / view_detail / view_logs / view_playbacks / view_notes / export / import_package / grant_expired
- `granted`：是否授权
- `userId`：操作人
- `grantId`：关联的授权单
- `accessedAt`：访问时间
...
审批员可通过 `GET /api/sensitive-desk/access-logs` 查看访问日志。

**6.10.9 命令行支持**

```bash
# 申请临时授权
node scripts/import-log.js --desk-apply <vaultBatchId> 张编辑 --reason "季度审计"

# 审批授权单
node scripts/import-log.js --desk-approve <grantId> 李审批

# 撤销授权单
node scripts/import-log.js --desk-revoke <grantId> 李审批 "不再需要"

# 查看所有授权单
node scripts/import-log.js --desk-grants

# 查看授权单详情
node scripts/import-log.js --desk-grant-detail <grantId> 张编辑

# 开启会话
node scripts/import-log.js --desk-open-session <grantId> 张编辑

# 验证会话
node scripts/import-log.js --desk-validate-session <sessionId>

# 查看保险箱批次详情（借阅台）
node scripts/import-log.js --desk-detail <vaultBatchId> 张编辑

# 导出审计包（借阅台）
node scripts/import-log.js --desk-export <vaultBatchId> 李审批 > desk-package.json

# 导入审计包（借阅台）
node scripts/import-log.js --desk-import desk-package.json 李审批

# 查看借阅台导入/导出记录
node scripts/import-log.js --desk-import-list

# 查看借阅台配置
node scripts/import-log.js --desk-config

# 触发过期授权回收
node scripts/import-log.js --desk-expire

# 查看访问日志
node scripts/import-log.js --desk-access-logs 李审批
```

**6.10.10 重启一致性保证**

- `sensitiveDeskGrants[]`：所有授权单
- `sensitiveDeskSessions[]`：所有会话
- `sensitiveDeskConfig`：配置（maxDurationMinutes / defaultDurationMinutes）
- `sensitiveDeskAccessLogs[]`：访问日志
- `sensitiveDeskImportPackages[]`：审计包导入导出记录
- 全部持久化到 `data/db.json`，使用原子写入（`.tmp` → rename）
- 重启后重新读取，授权单状态、会话有效性、权限判断结果完全一致
...
# 敏感审计借阅台
GET    /api/sensitive-desk/config                                        # 查看借阅台配置
PUT    /api/sensitive-desk/config                                        # 更新借阅台配置（仅审批员）
POST   /api/sensitive-desk/apply                                         # 申请临时授权（targetVaultBatchId+applicant+reason+durationMinutes）
POST   /api/sensitive-desk/:grantId/approve                              # 审批授权单（approver+notes）
POST   /api/sensitive-desk/:grantId/revoke                               # 撤销授权单（operator+reason）
POST   /api/sensitive-desk/:grantId/open-session                         # 开启会话（userId）
GET    /api/sensitive-desk/sessions/:sessionId/validate                  # 验证会话有效性
GET    /api/sensitive-desk/grants                                        # 授权单列表（?targetVaultBatchId=xx&applicant=xx&status=xx）
GET    /api/sensitive-desk/grants/:grantId                               # 授权单详情（?viewer=xx 控制脱敏）
GET    /api/sensitive-desk/vault-batches/:vaultBatchId/detail            # 查看批次详情（?viewer=xx&grantId=xx）
GET    /api/sensitive-desk/vault-batches/:vaultBatchId/logs              # 查看批次日志（?viewer=xx&grantId=xx）
GET    /api/sensitive-desk/vault-batches/:vaultBatchId/playbacks         # 查看回放记录（?viewer=xx&grantId=xx）
GET    /api/sensitive-desk/vault-batches/:vaultBatchId/notes             # 查看备注（?viewer=xx&grantId=xx）
GET    /api/sensitive-desk/vault-batches/:vaultBatchId/export            # 导出审计包（?viewer=xx&grantId=xx）
POST   /api/sensitive-desk/import                                        # 导入审计包（packageData+operator+conflictStrategy）
GET    /api/sensitive-desk/imported-packages                             # 借阅台审计包记录（?importer=xx&status=xx&fingerprint=xx）
GET    /api/sensitive-desk/access-logs                                   # 访问日志（仅审批员，?userId=xx&grantId=xx&action=xx）
POST   /api/sensitive-desk/expire-grants                                 # 触发过期授权回收（仅审批员）
```
...
- **敏感审计借阅台**：默认脱敏浏览（普通角色只看摘要、状态和最小元数据）；临时授权申请→审批→解锁完整明细→导出留痕全链路验证；授权单支持创建、审批、撤销、到期回收、原因记录和时长上限配置；撤销后已打开会话立刻失效；重启后权限结果不漂移；审计包导出再导入，重复导入同一批内容明确提示冲突并保留处理日志；旧缓存、筛选参数和组合查询不能绕过权限；测试覆盖默认脱敏、授权过期、重启恢复、导入冲突和越权访问
...
│   └── sensitive-desk.js     # 敏感审计借阅台：授权单、会话管理、临时授权、导出留痕
├── routes/
│   └── api.js                # RESTful API
├── public/                   # 前端（纯静态）
│   ├── index.html
